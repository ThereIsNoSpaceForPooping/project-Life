...
学习要点：
1. 多 Agent 系统通过分工协作提高任务处理能力
2. 路由器使用 LLM 进行意图分类
3. 每个专业 Agent 有独立的工具集
4. 使用条件边实现动态路由
...
def route_to_agent(state: MultiAgentState) -> str:
    ...
    学习要点：
    - 条件边根据这个函数的返回值路由
    - 返回值必须与 add_conditional_edges 的映射表匹配
    """
...
def researcher_node(state: MultiAgentState) -> dict:
    ...
    使用工具: search_knowledge, web_search
    
    学习要点：
    - 每个 Agent 可以绑定不同的工具集
    - 使用 SystemMessage 定义 Agent 角色
    """
